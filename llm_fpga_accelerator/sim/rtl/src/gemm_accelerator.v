// Simple GEMM Accelerator Top Module (fixed)
// Fixed: added missing base address registers (addr_a_base/addr_b_base/addr_c_base)
`timescale 1ns / 1ps
...
module gemm_accelerator #(
    ...
    // Base addresses for matrices (FIX: these were missing causing Verilator errors)
    reg [ADDR_WIDTH-1:0] addr_a_base;
    reg [ADDR_WIDTH-1:0] addr_b_base;
    reg [ADDR_WIDTH-1:0] addr_c_base;
    ...
    initial begin
        ...
        // Initialize base addresses (FIX: added)
        addr_a_base = 0;
        addr_b_base = 0;
        addr_c_base = 0;
    end
        ...
        if (!rst_n) begin
            ...
            if (s_axi_arvalid && !s_axi_rvalid) begin
                ...
                case (s_axi_araddr[7:0])
                    ...
                    8'h0C: s_axi_rdata <= addr_a_base; // added readback for base addresses
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (s_axi_awvalid && s_axi_wvalid && !s_axi_bvalid) begin
                ...
                8'h10: addr_a_base  <= s_axi_wdata; // FIX: now declared
                8'h14: addr_b_base  <= s_axi_wdata; // FIX: now declared
                8'h18: addr_c_base  <= s_axi_wdata; // FIX: now declared
                default: /* no-op to cover all cases */ ;
            endcase
